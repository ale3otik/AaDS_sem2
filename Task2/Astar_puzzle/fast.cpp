...
private:
    ...
    int GetSemenDistance(const PuzzleSolution * parent) const;
...
int GamePosition::GetSemenDistance(const PuzzleSolution * parent) const
{
    int answer = 0;
    ...
    {
        if(position[i] / size_of_game != i / size_of_game) ++answer;
        if(position[i] % size_of_game != i % size_of_game) ++answer;
    }
...
void GamePosition::SetPotential(PositionParams & position_params, const PuzzleSolution * parent) const
{
    ...
    potential += GetSemenDistance(parent);
